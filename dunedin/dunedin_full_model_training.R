# training full DK model post parameter tuning
# ethan whitman
# 9/11/23
...
workdir <- paste0(root, "Projects/ethan/pacni/performance_output/cluster/full_model_train/dk_gwr")
# ICCthr <- 0.75
...
alpha_best <- .214 # selected from 100 iterations of grid search with GWR
lambda_best <- .1 # selected from 100 iterations of grid search with GWR
...
for ( brainvarlist in brainvarlists ) {
  ...
  # regress sex and motion from data
  lm <- lm(data[,paste(behavvar)] ~ data$sex)
  data$behav_resids <- scale(lm$residuals) #### sure we want to scale this way?  
  snums <- data$snum
  ...
  enet <- train(
    ...
    tuneGrid = expand.grid(alpha = alpha_best, lambda = lambda_best)
  )
  predictions_enet <- enet %>% predict(data)
  coefs_enet <- coef(enet$finalModel, enet$bestTune$lambda)
  sum(as.numeric(coefs_enet) != 0)
  perf <- data.frame( method = "enet_full.sample ", brainvar = runname, behavvar=behavvar, nROIs = length(ROIs), iteration = 1, N=nrow(data),
                                  RMSE = RMSE(predictions_enet, data$behav_resids),
                                  Rsquare = R2(predictions_enet, data$behav_resids),
                                  MAE = MAE(predictions_enet, data$behav_resids),
                                  r = cor(predictions_enet, data$behav_resids ) ) 
  
  # haufe transform for coefficients per Tian and Zalesky NI 2021
  predictions_enet_train <- enet %>% predict(data) 
  enet_coefs_haufe <- c()
  N <- nrow(data)
  for (r in ROIs){ # loop through all edges
    r_std <- scale(data[,paste(r)])
    enet_coefs_haufe <- c( enet_coefs_haufe, sum(r_std * predictions_enet_train) / N )
  }
  
  ## save out everything
  outname <- paste0(gsub(" ", "_", gsub(":","_",date())), "_", round(runif(1,100,999),0))
  df <- data.frame(snum=snums, prediction_enet=predictions_enet)
  save(df, enet_coefs_haufe, ROIs_full, file=paste0(workdir,"/predictions_",outname,".Rdata"))
  save(perf, file=paste0(workdir,"/performance_",outname,".Rdata"))
  save(enet, file=paste0(workdir,"/model_",outname,".Rdata"))
